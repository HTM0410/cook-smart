#!/usr/bin/env node
/**
 * Chuẩn hoá unit kèm quy đổi quantity tương ứng (khi có công thức).
 *
 * Input:
 * - ingredient_unified_output/recipe_ingredient_mapping_unit_normalized.csv
 *
 * Output (không sửa file gốc):
 * - ingredient_unified_output/recipe_ingredient_mapping_unit_converted.csv
 * - ingredient_unified_output/recipe_ingredient_units_catalog_converted.csv
 *
 * Quy tắc:
 ...
 *   - kg -> g: q_new = q * 1000
 *
 * Nếu quantity không parse được số (vd: 'vừa đủ') thì giữ nguyên dòng.
 */
...
const IN_PATH = path.join(OUT_DIR, 'recipe_ingredient_mapping_unit_normalized.csv');
const OUT_PATH = path.join(OUT_DIR, 'recipe_ingredient_mapping_unit_converted.csv');
const UNITS_OUT = path.join(OUT_DIR, 'recipe_ingredient_units_catalog_converted.csv');
...
function formatQuantity(v) {
  // tránh quá nhiều số lẻ; giữ tối đa 3 chữ số thập phân
...
function main() {
  ...
  for (const r of data.rows) {
    ...
    // keep spoon units
    if (unit === 'muỗng canh' || unit === 'muỗng cà phê') {
      continue;
    }
  ...
  console.log('Converted rows:', converted, '| skipped (no numeric qty):', skippedNoQty);
}
